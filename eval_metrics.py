...
def eval_sysu_debug(distmat, q_pids, g_pids, q_camids, g_camids, query_img_path, gall_img_path, max_rank=20):
    """Evaluation with sysu metric
    Key: for each query identity, its gallery images from the same camera view are discarded.
    """
    num_q, num_g = distmat.shape
    ...
    return bad_q_ids, bad_q_labels, bad_g_labels, bad_q_paths, bad_g_paths
...
def eval_regdb_debug(distmat, q_pids, g_pids, bad_thre=0.5, max_rank=20):
    num_q, num_g = distmat.shape
    if num_g < max_rank:
        max_rank = num_g
        print("Note: number of gallery samples is quite small, got {}".format(num_g))
